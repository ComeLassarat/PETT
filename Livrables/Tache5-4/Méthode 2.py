...
def coos(E):            #fonction qui reprend la fonction fg mais que l'on utilie jute pour plot ce qui nous intéresse
...
def fginitial(E,Pp):   #Prend en entrée E=[Xo,V,Omega] et renvoye l'écart entre la trajectoire pointée et celle simulée dans plan2D, écart calculé seulement sur le critère du point initial

    Pos=simulation(E)   #Simule trajectoire; extrait liste coordonnées 3D Pos=[X1,Y1,Z1]
    Ps=echantillone(Pos)#Réduit liste 3D au bon nombre de points
    Pss=reelcam(Ps)      #Passe coordonnées 3D en 2D
    e=ecartinitial(Pp,Pss)#calcul l'écart entre le point initial des 2 trajectoires
    return(e)


def grad(E,s): #Fonction gradient comme pour la méthode 1. Soit on est dans le cas recherche du point initial (s=0) soit dans celui de la recherche de la vitesse initial (s=1)
    gra=np.zeros(9)
    if s==0:
        e=fginitial(E,Pp)
    if s==1:
...
#Pour le petit déplacement en vitesse, on prend des valeurs différentes car les vitesse selon les axes sont très différentes: la vitesse selon x est très impportant donc on prend dvx=0,2
    if s==1:
        ...
        kk=fg(KK,Pp)
        gra[5]=(kk-e)/0.005
    #Les résultats de la méthode 2 n'étant pas assez concluant on ne prend pas le temps de simuler l'impact de l'effet
...
def moindrecarré(Pp): #Fonction principale renvoyant le vecteur E théorique qui doit correspondre à celui à l'orignie de la trajectoire pointée
    ...
    FG=fginitial(E,Pp)
    s=0
    while FG>0.02:   #recherche du point initial jusqu'à ce qu'un certain seuil soit franchit.
        print(n)
        print(FG)
        Q=grad(E,s)    #calcul gradient
        E=E-gamma*Q    #change E par la méthode du gradient
        erreurinitial.append(FG)
        FG=fginitial(E,Pp)    #calcul erreur nouvelle trajectoire
        if FG>0.02 and FG<0.04:   #Pas pour la méthode du gradient est adapté en fonction de l'erreur
            gamma=0.08
        if FG<0.02:
            gamma=0.008
        if FG<0.009:
    ...
    print(y)
    print(FGG)
    while (FGG>0.005 and m<y) or abs(FGG-erreur[-1])>0.06: #recherche de la vitesse initiale jusqu'à ce qu'un certain nombre d'itération soit réalisé
        print(m)
        print(FGG)
        Q=grad(E,s)    #calcul gradient
        E=E-delta*Q   #change E par la méthode du gradient
        erreur.append(FGG)
        FGG=fg(E,Pp)     #calcul erreur nouvelle trajectoire
        m=m+1
        delta=choixdelta(FGG)  #Pas pour la méthode du gradient est adapté en fonction de l'erreur
