...
def ecart(Pp,Pss,i):              #Fonction calculant écart entre trajectoires 2D pointées et celle simulée
    e=0
    for k in range (i):
...
def fg(E,Pp,i):              #Prend en entrée E=[Xo,V,Omega] et renvoye l'écart norme entre la trajectoire pointée et celle simulée dans plan2D
    Pos=simulation(E)   #Simule trajectoire; extrait liste coordonnées 3D Pos=[X1,Y1,Z1]
    Ps=echantillone(Pos)
    Pss=reelcam(Ps)
    #print(Pp)
...
def choixdelta(FGG):    #fonction renvoyant le pas pour la methode du gradient en fonction de l'erreur
    if FGG<0.1:
        d=0.1
    if FGG>0.1 and FGG<0.3:
        d=0.2
    if FGG>0.3 and FGG<0.5:
        d=0.4
    if FGG>0.5 and FGG<0.7:
        d=0.6
    if FGG>0.7 and FGG<1.5:
        d=0.7
...
def moindrecarré(Pp):
    ...
    for i in range(1,len(Pp[0])):       #pour chaque point on va adapter la vitesse initiale par la méthode du gradient
        ...
        m=0
        erreur.append(FGG)
        while ((FGG>0.01) and (m<y)) or (abs((FGG-erreur[-2]))>0.005):
            print(m)
            print(FGG)
            Q=grad(E,s,i)
